...
import { Button, Pane, Text, Heading, TextInput, Icon, Dialog } from 'evergreen-ui'
import { Mutation, Query } from 'react-apollo'
import { gql } from 'apollo-boost'
// import { ME_QUERY } from '../../../../graphql/queries'

const LogInModal = ({ children }) => (
	<Component initialState={{ isShown: false, isLoading: false, email: '', message: null }}>
		{({ state, setState }) => (
			<Pane>
				<Dialog
					isShown={state.isShown}
					onCloseComplete={() =>
						setState({ isShown: false, isLoading: false, email: '', message: '' })
					}
				...
				>
					<div className="flex flex-column items-center">
						...
						<Mutation
							...
							onCompleted={() =>
								setState({ isLoading: false, message: 'Please check your Inbox!', success: true })
							}
						>
							{(mutation, { loading, error }) =>
								!state.success ? (
									...
									<div className="flex items-center justify-center mt4 mb3">
										<Text paddingRight={16}>{state.message && state.message}</Text>
